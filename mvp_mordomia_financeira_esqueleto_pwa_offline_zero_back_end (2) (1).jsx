...
function grauDeMeta(renda:number): Grau {
  if (renda <= 1000) return 'SUB';
  if (renda <= 4000) return 'M1';
  if (renda <= 7000) return 'M2';
  if (renda <= 10000) return 'M3';
  if (renda <= 13000) return 'M4';
  return 'M5';
}
...
function CaptarAssistentes({onCreate}:{onCreate:(l:Partial<Lanc>)=>void}){
  ...
  const pctPacto = FIDELIDADE_DETALHE[grau].PACTO / 100;
  ...
  function confirmarDizimo(){
    ...
    onCreate({
      item: 'Dízimo', mov: 'Saída', valor: dizimo,
      categoria: 'FIDELIDADE', subcategoria: 'Dízimo (10%)', origem: 'Usuário', destino: 'Recebedor', pagador: 'Usuário', recebedor: 'Recebedor', dataAquis: new Date().toISOString().slice(0,10), dataMov,
    });
  ...
  function confirmarOferta(){
    if(base<=0){ push('Informe a base do mês (> 0)'); return; }
    onCreate({
      item: 'Oferta | Pacto', mov: 'Saída', valor: oferta,
      categoria: 'FIDELIDADE', subcategoria: 'Oferta | Pacto', origem: 'Usuário', destino: 'Recebedor', pagador: 'Usuário', recebedor: 'Recebedor', dataAquis: new Date().toISOString().slice(0,10), dataMov,
    });
...
function ScreenCaptar(){
  ...
  function save(){
    ...
    const full: Lanc = {
      ...
      mov: f.mov!, valor: Number(f.valor), modalidade:f.modalidade as Modalidade|undefined,
    ...
    if(isDuplicate(items, full)){
      push('Atenção: possível duplicata (intervalo ±3 dias).');
    }
          ...
          <tbody>
            {items.map(i=> (
              <tr key={i.internalId}>
                ...
                <td style={{padding:'6px 4px'}}>{BRL(i.valor)}</td>
    ...
    <div style={{display:'grid', gap:8}}>
      <Field label='Escolha o item a reembolsar'>
        <Select value={sel} onChange={e=> setSel(e.target.value)}>
          <option value=''>—</option>
          {items.map(i=> <option key={i.internalId} value={i.internalId}>{i.indice} — {i.item} ({BRL(i.valor)})</option>)}
        </Select>
      </Field>
...
function ScreenFidelidade(){
  ...
  const cat = PERCENTUAIS[grau];
  const fid = FIDELIDADE_DETALHE[grau];
...
function runTests(){
  ...
  (['SUB','M1','M2','M3','M4','M5'] as Grau[]).forEach(g => {
    const total = Math.round(Object.values(PERCENTUAIS[g]).reduce((a,b)=>a+b,0)*1000)/1000;
    tests.push({ name:`Soma categorias = 100% [${g}]`, pass: Math.abs(total - 100) < 1e-6, details:`total=${total}` });
    const fid = Math.round(Object.values(FIDELIDADE_DETALHE[g]).reduce((a,b)=>a+b,0)*1000)/1000;
    tests.push({ name:`Soma fidelidade detalhada = FIDELIDADE [${g}]`, pass: Math.abs(fid - PERCENTUAIS[g].FIDELIDADE) < 1e-6, details:`det=${fid} vs cat=${PERCENTUAIS[g].FIDELIDADE}` });
    tests.push({ name:`Dízimo = 10% [${g}]`, pass: Math.abs(FIDELIDADE_DETALHE[g].DIZIMO - 10) < 1e-6 });
  });
  const progFid = (['SUB','M1','M2','M3','M4','M5'] as Grau[]).map(g=>PERCENTUAIS[g].FIDELIDADE);
  tests.push({ name:'Fidelidade não decresce por grau', pass: progFid.every((v,i,a)=> i===0 || v>=a[i-1]), details:progFid.join(' ≤ ') });
  const progEmer = (['SUB','M1','M2','M3','M4','M5'] as Grau[]).map(g=>PERCENTUAIS[g].EMERGENCIAL);
  ...
  const boundaries: [number, Grau][] = [[1000,'SUB'],[1001,'M1'],[4000,'M1'],[4001,'M2'],[7000,'M2'],[7001,'M3'],[10000,'M3'],[10001,'M4'],[13000,'M4'],[13001,'M5']];
    ...
    <ToastProvider>
      <div style={{minHeight:'100vh', background:'#f8fafc', color:'#0f172a'}}>
        ...
        <main style={{maxWidth:960, margin:'0 auto', padding:16}}>
          ...
          {['MONITOR','EXPORTAR','AJUSTES','BACKUP','LICENCA'].includes(route) && (
            <div>
